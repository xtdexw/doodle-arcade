...
3. **regress.js 精灵占位符未替换**：`scripts/regress.js:28` 仍传已废弃的 `SPRITE_DATA` 变量（模板无此键，死代码），且落盘 `regress-*.html` 不做 `__DOODLE_SPRITE__` 替换（routes/api.js 有 `substituteSprite`，脚本没有）→ 回归产物精灵图必坏，无法做「精灵不变形」验收。建议：对齐 api.js 行为。另：9adbba0 的「关思考+32k」只在 routes/api.js chatHtml 生效，regress.js 的 generate 调用未沾光，仍会踩思维链耗尽预算问题。
...
| 2026-09-24 | generate.md（未改，达标） | 无改动——设计精调+路由关思考（7fef1a8/9adbba0）后现版本即达标 | 旧参数下（复杂设计+思考开）：4 次尝试 0 次产出正文（非流式超时×2、流式中途 429×1、纯推理 1.7 万字被掐×1） | 路由同参（关思考+32k+0.7）2 次全过契约静态 8/8（290s/963 行、209s/1056 行，配色零越界）；run1 动态验证：加载即开始、空格跳（角色中心 y 429→393）、真触屏跳零报错、撞击→结束画面（死因/得分/重开按钮/空格提示）、空格重开、自动跳跃 16s 得分 2→9 爬升、精灵无变形、DOODLE_GAME 三函数可调。**达标（n=2）** |
| 2026-09-24 | repair.md（未改，达标） | 无改动 | 未验证 | 真机破坏性测试：将成片游戏的主循环函数改名（update→updateBROKEN）+ 注入对应报错信息，POST /api/repair → 122s 返回修复版：update 函数恢复、无 BROKEN 残留、DOODLE_GAME 契约完好、体积一致。产物存档 games/verify-repaired.html。**达标（n=1）** |
| 2026-09-24 | revise.md（未改，达标） | 无改动 | 未验证 | 真机指令测试：「跳跃高度提高百分之三十，其余保持不变」→ 124s 返回修改版：DOODLE_GAME 契约完好、占位符无残留、体积仅 +1 字节级差异（最小化改动）。产物存档 games/verify-revised.html。**达标（n=1）** |
| 2026-09-24 | repair.md（未改，达标） | 无改动——现版本即达标 | 制造坏 HTML：删 `spawnObstacle` 函数（保留调用点），浏览器实测报 `spawnObstacle is not defined`（即沙箱会捕获并回喂的错误） | 155s 修复成功：函数恢复（实现略异但语义等价）、占位符原样保留、无 base64 泄漏、回填后精灵在；动态验证零报错、计分正常爬升、契约三函数在。**改动最小化实测：difflib 相似度 0.994，变更 ~11 行且全部位于被删函数位置（外科手术式）**。**达标** |
