...
2. **generate 长生成必须流式**：`lib/ark.js` 非流式调用，推理型模型（doubao-seed-evolving）写大 HTML 时服务端长时间不回响应头，超过 undici 默认 headersTimeout（5min）连接必挂（`UND_ERR_HEADERS_TIMEOUT`；curl 非流式 9.7min 仍 0 字节）。已验证 `stream:true` SSE 可行（`data: {choices:[{delta:{content|reasoning_content}}]}`，`[DONE]` 收尾）。建议：ark.js chat 增加流式分支，聚合 delta 后返回，接口签名不变。
