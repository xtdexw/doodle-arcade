...
3. 主角精灵通过占位符引用：在需要精灵图的地方（img 的 src 或 JS 字符串常量）写 __DOODLE_SPRITE__，系统会在生成后自动替换为真实图片数据；等比绘制，不拉伸变形；
...
精灵图说明：占位符 __DOODLE_SPRITE__ 对应一张以涂鸦角色为主体的 PNG（透明背景）。严禁在输出中编写任何 base64 图片数据，只使用该占位符。
